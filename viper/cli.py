...
def run_snippet(source: str, filename: str = "<viper -c>") -> int:
    """Run a string of Viper code (from `-c CODE` or piped stdin)."""
    source = source.lstrip("﻿")   # Windows/PowerShell pipes prepend a BOM
    try:
...
def _build_prelude() -> str:
    """Source text of the whole Viper stdlib prelude, for a self-contained
    build. Inlines each helper's actual source so the output needs no
    viper-lang install; falls back to importing if source is unavailable."""
    import inspect
    from . import std
    try:
        blocks = [inspect.getsource(fn) for fn in std.PRELUDE_SUPPORT]
        blocks += [inspect.getsource(fn) for fn in std.PRELUDE.values()]
        return "".join(blocks)
    except (OSError, TypeError):
        return "from viper.std import *  # Viper stdlib prelude\n"
...
def build_file(path: str, output: str | None = None) -> int:
    ...
    header = (
        ...
        f"# Source: {path}\n"
        "# This file is self-contained: the Viper stdlib prelude is inlined below.\n"
        f"{_build_prelude()}"
        "# --- end prelude ---\n"
    )
...
def main(argv=None) -> int:
    ...
    ap.add_argument("-c", metavar="CODE", dest="code",
                    help="run a snippet of Viper code, then exit")

    # Shebang / direct-run support: `viper script.vp` == `viper run script.vp`
    raw = sys.argv[1:] if argv is None else list(argv)
    if raw and raw[0].endswith(".vp"):
        return run_file(raw[0])
    # One-liner: `viper -c 'print(sha256("hi"))'`
    if raw and raw[0] == "-c":
        if len(raw) < 2:
            print("error: -c requires a code argument", file=sys.stderr)
            return 2
        return run_snippet(raw[1])
    # Stdin: `echo 'print(now())' | viper -`
    if raw == ["-"]:
        # Read bytes and decode as utf-8-sig: this strips a UTF-8 BOM (which
        # PowerShell/Windows pipes prepend) and ignores the console codepage.
        data = sys.stdin.buffer.read()
        return run_snippet(data.decode("utf-8-sig"), filename="<stdin>")

    args = ap.parse_args(argv)
    if args.code is not None:
        return run_snippet(args.code)
